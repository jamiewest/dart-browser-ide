...
// The shell loads this as a real worker sitting beside its `vendor/`
// directory, so the relative paths resolve. The browser IDE cannot: its
// extension host re-hosts every worker through a generated blob shim, and a
// relative `importScripts` there resolves against the blob rather than the app
// (see browser-ide/public/worker_base.mjs). It concatenates the two libraries
// ahead of this file instead — which is exactly what `importScripts` would
// have done — so they may already be here.
if (typeof globalThis.git === 'undefined') {
  importScripts(
      'vendor/isomorphic-git.umd.min.js', 'vendor/isomorphic-git-http.umd.js');
}
...
// The row carries three independent facts and `classifyRow` collapses them,
// which is right for a single list of changes and wrong for a two-group SCM
// view: `[1,2,3]` is a file whose staged changes have been edited again, and
// it belongs in *both* groups with different diffs. So it is classified twice,
// once per group. Meanings are isomorphic-git's: head 0 absent / 1 present;
// workdir 0 absent / 1 same as HEAD / 2 differs; stage 0 absent / 1 same as
// HEAD / 2 same as workdir / 3 differs from both.

/// What the index has that HEAD does not, or null.
function stagedStatus([, head, workdir, stage]) {
  if (head === 0) return stage === 0 ? null : 'added';
  if (stage === 0) return 'deleted';
  if (stage === 3) return 'modified';
  // `2` means "same as the working tree", which is only a staged change when
  // the working tree itself differs from HEAD.
  if (stage === 2 && workdir === 2) return 'modified';
  return null;
}

/// What the working tree has that the index does not, or null.
function unstagedStatus([, head, workdir, stage]) {
  if (workdir === 0) return stage === 0 ? null : 'deleted';
  if (workdir === 1) return null;
  if (stage === 0) return head === 0 ? 'untracked' : 'modified';
  if (stage === 2) return null; // the index already matches the working tree
  return 'modified';
}
...
const ops = {
  ...
  async status(args) {
    ...
    for (const row of matrix) {
      const staged = stagedStatus(row);
      const unstaged = unstagedStatus(row);
      // `status` keeps its old meaning and its old callers; the fallback only
      // fires for combinations classifyRow declines to name (a staged delete
      // whose file was then restored, say), which used to vanish entirely.
      const status = classifyRow(row) ?? staged ?? unstaged;
      if (!status) continue;
      entries.push({ path: row[0], status, staged, unstaged });
    }
    return entries;
  },

  /// Stages [paths]. A deleted file cannot be `add`ed — staging its deletion
  /// is `remove` — so which one to call depends on the working tree.
  async stage(args) {
    const dir = projectDir(args);
    for (const filepath of args.paths ?? []) {
      let present = true;
      try {
        await fsPromises.stat(dir + '/' + filepath);
      } catch (e) {
        if (!isNotFound(e)) throw e;
        present = false;
      }
      if (present) await git.add({ fs, dir, filepath });
  ...
  /// Commits what is in the index and nothing else — unlike [commit], which
  /// stages the whole working tree first. Reports `{nothing: true}` rather
  /// than writing an empty commit.
  async commit_index(args) {
    const dir = projectDir(args);
    const matrix = await git.statusMatrix({ fs, dir });
    if (!matrix.some((row) => stagedStatus(row) !== null)) {
      return { nothing: true };
    }
  ...
  /// The *index* version of one file.
  ///
  /// A row in Staged Changes means "HEAD vs index", not "HEAD vs working
  /// tree" — for a file that is staged and then edited again the two are
  /// different, and showing the second one there is simply wrong. The index
  /// is not a tree that `readBlob` can be pointed at directly, so it is walked
  /// for the one path.
  async index_file(args) {
    const dir = projectDir(args);
    if (typeof git.walk !== 'function' || typeof git.STAGE !== 'function') {
    ...
    try {
      const found = await git.walk({
        ...
        map: async (filepath, [entry]) => {
          if (filepath !== args.path || !entry) return undefined;
          const oid = await entry.oid();
          return oid ? { oid } : undefined;
        },
      });
      const oid = found?.[0]?.oid;
      if (!oid) return { found: false, text: '' };
      const { blob } = await git.readBlob({ fs, dir, oid });
      const text = utf8Decoder.decode(blob);
      if (text.includes('\0')) return { found: false, binary: true, text: '' };
  ...
  /// The HEAD version of one file, for a quick-diff gutter.
  ///
  /// `found` is separate from `text` on purpose: an empty string is a real
  /// file content, and conflating it with "not in HEAD" would render every
  /// line of a new file as an unchanged one.
  async head_file(args) {
    ...
    try {
      ...
      // No commits yet, or the path is not in the HEAD tree.
      return { found: false, text: '' };
    }
  },
